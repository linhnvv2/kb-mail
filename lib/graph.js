import { getGraphToken } from "./settings.js";

const GRAPH = "https://graph.microsoft.com/v1.0";

function token() {
  // Ưu tiên token admin đã lưu (data/settings.json), fallback về .env
  const t = getGraphToken();
  if (!t) throw new Error("Thiếu GRAPH_TOKEN (cập nhật trong trang Quản trị hoặc .env)");
  return t;
}
...
// Mail CHƯA ĐỌC trong một folder (vd "Inbox"), nhận từ mốc `sinceIso`.
// Mặc định: từ 0h hôm nay (giờ máy chủ) -> chỉ lấy mail trong ngày hôm nay.
// Dùng cho chế độ rule quét theo inbox (không cần gắn cờ).
export async function fetchUnreadFolderMessages(pathStr = "Inbox", sinceIso) {
  const folderId = await findFolderByPath(pathStr || "Inbox");
  let since = sinceIso;
  if (!since) {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
  ...
  let url = `/me/mailFolders/${folderId}/messages?${SELECT}&$filter=isRead eq false and receivedDateTime ge ${since}&$orderby=receivedDateTime desc&$top=50`;
  const out = [];
  while (url) {
    const page = await graphFetch(url);
    out.push(...page.value.map(mapMessage));
